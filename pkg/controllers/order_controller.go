...
func GetRestaurantOrders(rid int64) (*[]models.CustomQuery, gin.H) {
	...
	results, err := db.Query("SELECT order_item_id, prod_id, prod_name, prod_desc, table_id, prod_count, order_status, order_created_at FROM products JOIN orders ON `orders`.`prod_id` = products.prod_id WHERE `orders`.`rest_id` = (?) AND (`orders`.`order_status` = 'To do' OR  `orders`.`order_status` = 'In progress' OR `orders`.`order_status` = 'Completed')", rid)
	if err != nil {
	...
	for results.Next() {
		...
		err = results.Scan(&cq.OrderItemID, &cq.ProductID, &cq.ProductName, &cq.ProductDescription, &cq.TableID, &cq.Quantity, &cq.Status, &cq.OrderCreatedAt)
		if err != nil {
